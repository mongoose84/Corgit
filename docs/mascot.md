...
| 3 | **Working** | `pose-working` | While an operation is running — a sweep, or a write the user asked for (SPEC §13) | Trotting, stick in mouth, motion lines behind. Held for an indefinite duration |
...
| Mini working, 18px | `panes/GraphPane.svelte` | A write is running on the selected repo (SPEC §13, *Work in progress*) — beside the label naming it, in the slot the repo name otherwise holds. The same "takes the slot back when it lands" mechanic as the row above, and the reason Working's meaning is now *an operation is running* rather than *the sweep is running*: it was already the only pose reporting live work, and a second glyph for foreground writes would have said the same thing twice |
