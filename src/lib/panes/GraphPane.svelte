<script lang="ts">
  ...
  import BranchFind from './BranchFind.svelte';
  ...
  $effect(() => {
    ...
    if (id !== lastRepoId) {
      ...
      pullPrompt = null;
      // The search does not survive a repo change either (§5.3) — a box
      // reappearing over a different repo's history is a search that has
      // quietly changed its subject. `BranchFind` clears the query, the cursor
      // and the out-of-reach line for the same reason; this half is here
      // because `findOpen` is this pane's state and outlives the box, which
      // unmounts while the new repo's first page loads.
      findOpen = false;
    }
  });
  ...
  //
  // The box itself is `BranchFind.svelte`; what stays here is the half that is
  // the pane's own — whether it is open (the header button paints itself from
  // it), and the two steps of a jump that touch the graph's pages, its
  // selection and this pane's scroll box.
  let findOpen = $state(false);
  let branchFind: ReturnType<typeof BranchFind> | undefined = $state();

  /** Read pages until `ref`'s tip is loaded and select it, answering with its
   *  row index — or -1 if `graph.reveal` gave up inside §5.3's page cap. */
  async function revealRef(ref: RefBadge): Promise<number> {
  ...
  function onKeydown(event: KeyboardEvent) {
    ...
    if ((event.ctrlKey || event.metaKey) && event.key === 'f') {
      ...
      if (typing && !(branchFind?.ownsTarget(target) ?? false)) return;

      event.preventDefault();
      void branchFind?.openBox();
      return;
    }
...
<Pane title="Graph">
  ...
  {#snippet actions()}
    ...
    <button
      ...
      onclick={() => (findOpen ? branchFind?.closeBox() : void branchFind?.openBox())}
  ...
  <div class="view" class:hidden={diff.view !== 'graph'}>
    ...
    {:else}
      ...
      <div class="graph-body">
        <BranchFind
          bind:this={branchFind}
          bind:open={findOpen}
          repoId={repos.selectedId}
          {currentBranch}
          reveal={revealRef}
          centre={scrollToRow}
        />
